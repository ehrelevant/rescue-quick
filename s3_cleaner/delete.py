...
def delete_photos(prefix: str = "camera", interval: dict[str, int] = {"hours": 1, "minutes": 0, "seconds": 0}) -> int:
    """
    ...
    """
    time_passed = datetime.now(timezone.utc) - timedelta(hours=interval.get("hours", 0), minutes=interval.get("minutes", 0), seconds=interval.get("seconds", 0))
    response: dict[str, typing.Any] = s3.list_objects_v2(Bucket=BUCKET_NAME, Prefix=prefix)
...
def main():
    while True:
        ...
        try:
            ...
            processed_deleted = delete_photos(prefix=PROCESSED_OBJ, interval=TIME_INTERVAL)
        except Exception as e:
            print(f"Error: {e}")
