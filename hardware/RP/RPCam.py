...
def get_flood_status() -> bool:
    ...
    try:
        ...
        return (
            ...
            and str(response_json['indicator']).lower() == 'true'
        )
...
def send_image(picam):
    ...
    # Send the image
    try:
        payload = {'image': encodedImage, 'content_type': 'image/jpeg'}
        ...
        response = requests.post(
            FULL_POST_IMG_PATH, headers=headers, data=json.dumps(payload)
        )
...
def send_health():
    try:
        payload = {'state': 'alive', 'content_type': 'text/plain'}
        ...
        print('POST request ready to be sent')
        response = requests.post(
            FULL_POST_HEALTH_PATH, headers=headers, data=json.dumps(payload)
        )
        print(f'POST status: {response.status_code}')
        print(f'Response: {response.text}')
    except Exception as e:
